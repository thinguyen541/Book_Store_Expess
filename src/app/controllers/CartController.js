...
class CartController {
            ...
            .then(async (cart) => {
                ...
                for (let i = 0; i < cart_obj.books.length; i++) {
                    if (cart_obj.books[i])
                        books.push(
                            Mongoose(
                                await Book.findById(cart_obj.books[i].bookid),
                            ),
                        );
                }
    ...
    async addBookToCart(req, res, next) {
        ...
        Book.findById(req.params.id.split('?_method')[0])
            .then((book) => {
                ...
                for (let i = 0; i < updatedcart.books.length; i++) {
                    if (
                        updatedcart.books[i] != null &&
                        updatedcart.books[i].bookid ===
                            req.params.id.split('?_method')[0]
                    ) {
                ...
                if (check == 0) {
                    Cart.findOneAndUpdate(
                        ...
                        {
                            $push: {
                                books: { bookid: book_obj._id, quantity: 1 },
                            },
                        },
